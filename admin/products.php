...
if (isset($_GET['delete'])) {
    ...
    $stmt->execute([$ตัวแปรค่ำที่องกำbindparam]);
    header("Location: products.php");
    exit;
}
// ดงึรำยกำรสนิคำ้
...
// ดึงหมวดหมู่
$categories = $conn->query("SELECT * FROM products")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="th">
            ...
            <div class="col-md-2">
                <select name="category_id" class="form-select" required>
                    ...
                    <?php foreach ($categories as $cat): ?>
                        <option value="<?= $cat['category_id'] ?>"><?= htmlspecialchars($cat['category_id']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-12">
            ...
            <?php foreach ($products as $p): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="edit_product.php?id=<?= $p['product_id'] ?>" class="btn btn-sm btnwarning">แก้ไข</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
